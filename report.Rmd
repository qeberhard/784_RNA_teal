...
The initial matrices were stored in `se_K562.rda` and `se_HEPG2.rda`.
...
For the clustering analysis, we use a filtered SummarizedExperiment. It contains data for just mRNAs and lncRNAs with nonzero eCLIP data. We narrow our focus to these two types of RNAs. The reasons are that they are very well represented in our dataset, and that unlike mRNAs, the functions of almost all lncRNAs remain a mystery. If we see that similar mRNAs are bound by similar proteins, then perhaps this might also hold true for lncRNAs.

The first step is to load the SummarizedExperiment. In "assay(se)", the rows are the ENSEMBL gene ID's and the columns are sample ID's (note that each sample in an eCLIP experiment with a single, unique protein).

```{r warning=F}
load("C:/Users/anastacia/Desktop/784_RNA_teal/developing_se/K562_se/se2_K562.rda")
se2
```

Judging by the counts data, some proteins heavily bind our RNAs, and others bind more lightly. Unsurprisingly (in hindsight), working with the unscaled data produced poor clustering results because of this wide spread. As such, we scale the counts data by column sums, normalizing the protein counts. In addition, we also remove any RNAs relatively unbound by proteins (RNAs with row sums less than 100).
...
From the plot above, we see that YBX3 is way out in the distance. Perhaps this is why PC1 and PC2 explain such a small percent of the variance. What happens to to the PCA results when we remove YBX3?
...
### a = lncRNAs bound by proteins in    top cluster
### b = lncRNAs bound by proteins in bottom cluster
### c =   mRNAs bound by proteins in    top cluster
### d =   mRNAs bound by proteins in bottom cluster
library(vioplot)
vioplot(lnc.top, lnc.bot, prt.top, prt.bot,
        names=c('a','b','c','d'))
title(ylab="mean proteins counts for an RNA across a cluster")
```
...
Interesting that we see two high density areas on the PCA plot for mRNAs. Let's explore these areas further with a GO-term enrichment analysis. Note that we did try to repeat this analysis for lncRNAs, but not surprisingly, there aren't enough GO terms assigned to lncRNAs.
...
Step 3: Using data for 103 available eCLIP experiments in HEPG2 cells, figure out how similar the 103 eCLIP proteins are in terms of their binding to mRNAs and lncRNAs.
...
Step 8: For the two high-density clusters on the mRNA-specific PCA, explore further with a GO-term enrichment analysis.
...
Looking at the the PCA results for the 135 eCLIP proteins in K562 cells, we see that though the variance explained by PC1 and PC2 is small, YBX3 clearly stands out. In HEPG2 cells, 103 eCLIP experiments were carried out, which could explain the different in PCA results for the two cell lines. YBX3 eCLIP was not conducted in HEPG2.
...
Perhaps the most intriguing result of the clustering analysis is that mRNAs and lncRNAs cluster somewhat independently based on their binding of proteins. Unlike mRNAs, the functions of most lncRNAs are completely unknown. The area where the mRNA and lncRNAs clusters overlap most contains mRNAs enriched for ncRNA processing, ribosome biogenesis, rRNA metabolic process, tRNA metabolic process, rRNA processing, RNA modification, ... Perhaps the lncRNAs also in this overlap region have similar roles.

Curious to see where Xist is located in the "PCA Results for lncRNAs plot" for K562, we found that it is right beside the lncRNA Xact. This pair stand relatively apart from the other lncRNAs. Whereas Xist is responsible for silencing an X chromosome in XX females, Xact is involved in X chromosome activation. The two work antagonistically (Vallot et al, 2017). We identified other pairs of lncRNAs and attempted to see if they were involved in similar processes, but there were too many unknowns to make any headway. As more is learned about lncRNAs, perhaps given a plot similar to the PCA, a "connect-the-dot" approach can be used. Specifically, if an unknown lncRNA is positioned very nearby a group of known lncRNAs, then perhaps it could give insight into the possible function of the unknown lncRNA.
